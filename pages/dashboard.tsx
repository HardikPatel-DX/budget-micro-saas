...
function getSupabaseBrowserClient(): SupabaseClient | null {
  // Guard against build-time / server-side execution
  if (typeof window === "undefined") return null;

  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const anon = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

  // If env vars are missing, do NOT throw. Show a friendly message instead.
...
const Dashboard: NextPage = () => {
  ...
  async function fetchSummary() {
    ...
    try {
      if (!supabase) {
        setError(
          "Missing Supabase env vars. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in your environment (and in Vercel)."
        );
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    // If supabase is missing, we already show a clean error instead of crashing build.
    fetchSummary();
  ...
  if (!data)
    return (
      <div style={styles.page}>
        <div style={styles.card}>No data</div>
      </div>
    );
    ...
    <div style={styles.page}>
      <header style={styles.header}>
        ...
        <div style={{ fontSize: 12, color: "#9CA3AF" }}>
          {s.generated_at} {s.cached ? "(cached)" : ""}
        </div>
      </header>
      ...
      <section style={styles.grid}>
              ...
              <div key={w.weekStart} style={{ flex: 1, margin: "0 4px", textAlign: "center" }}>
                ...
                >
                  <div
                    style={{
                      width: "80%",
                      height: `${Math.min(100, (w.amount / (s.tiles.weeklyAvgSpend26 * 3 || 1)) * 100)}%`,
                      background: "#ef4444",
                      borderRadius: 2,
                    }}
                  />
        ...
        <div style={styles.card}>
          ...
          <ol>
            {s.topPayees.map((p: any) => (
              <li key={p.payee}>
                {p.payee}: ${small(p.amount)}
              </li>
            ))}
          </ol>
        </div>
        ...
        <div style={styles.card}>
          ...
          <ul>
            ...
            {s.recurring.map((r: any) => (
              <li key={r.payee}>
                {r.payee}, ${small(r.avg_amount)}, next: {r.next_date || "unknown"}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div style={styles.card}>
          ...
          <ul>
            ...
            {s.upcomingBills.map((b: any) => (
              <li key={b.payee}>
                {b.payee}: ${small(b.amount)}, {b.next_date || "N/A"}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div style={styles.cardFull}>
          <h3 style={styles.sectionTitle}>Savings Scenarios (weekly and projections)</h3>
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, color: "#9CA3AF" }}>
                Method used: {s.savingsScenarios.methodUsed} (weeklyNetFlow: ${small(s.savingsScenarios.weeklyNetFlowValue)})
              </div>
              ...
              <table style={{ width: "100%", marginTop: 8 }}>
                ...
                <tbody>
                  <tr>
                    <td>Conservative (5%)</td>
                    <td>${small(s.savingsScenarios.conservative.weekly)}</td>
                    <td>${small(s.savingsScenarios.conservative.projection12)}</td>
                    <td>${small(s.savingsScenarios.conservative.projection26)}</td>
                  </tr>
                  <tr>
                    <td>Moderate (10%)</td>
                    <td>${small(s.savingsScenarios.moderate.weekly)}</td>
                    <td>${small(s.savingsScenarios.moderate.projection12)}</td>
                    <td>${small(s.savingsScenarios.moderate.projection26)}</td>
                  </tr>
                  <tr>
                    <td>Aggressive (15%)</td>
                    <td>${small(s.savingsScenarios.aggressive.weekly)}</td>
                    <td>${small(s.savingsScenarios.aggressive.projection12)}</td>
                    <td>${small(s.savingsScenarios.aggressive.projection26)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <footer style={{ color: "#9CA3AF", fontSize: 12, textAlign: "center", marginTop: 20 }}>
        Tip: This dashboard calls /api/dashboard-summary, cached server-side for 5 minutes.
      </footer>
    </div>
  );
};
