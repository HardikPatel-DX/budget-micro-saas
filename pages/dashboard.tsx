...
const Dashboard: NextPage = () => {
  ...
  function extractTokenFromUrlOrCookies(): string | null {
    try {
      ...
      const ck = (typeof document !== 'undefined' && document.cookie) ? document.cookie.split('; ') : [];
      for (const c of ck) {
        if (/sb-.*-auth-token|sb-access-token|sb-.*-auth-token-code-verifier/i.test(c)) {
          const parts = c.split('=');
          if (parts.length >= 2) {
  ...
  async function fetchSummary() {
    ...
    try {
      ...
      if (!token) {
        ...
        if (token) {
          console.info('Dashboard: using token extracted from URL/cookie (temporary fallback).');
        }
      }

      if (!token) {
        setError('Not authenticated. No session found. Please sign in (check Supabase redirect settings).');
        setLoading(false);
        return;
      }
      ...
      if (!r.ok) {
        if (r.status === 401) {
          setError('API returned 401 Unauthorized. Token may be invalid or expired. Try signing in again or check Supabase redirect settings.');
        } else {
